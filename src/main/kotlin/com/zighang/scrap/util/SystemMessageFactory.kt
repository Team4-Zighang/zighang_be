...
class SystemMessageFactory {
    ...
    companion object{
        ...
        private val analysisCareer : String = """
            아래 저장된 채용 요약 데이터를 분석해서 career정보를 추출해주세요.
            규칙:
            1. 출력은 **JSON 형식**으로만 하고, 다른 key를 만들지 마세요.
            2. JSON key는 반드시 아래와 일치해야 합니다:
               - career : 1,2,3 (1년차 ~ 3년차)
            3. career를 숫자 리스트로 변환하는 규칙은 다음과 같다. 여기서 n은 0 이상의 정수이다.
                - "n년차 이상"이면 결과는 "n, n+1, ..., 10"이다. (최대 10까지 표기)
                - "n년차 ~ n+3년차"이면 결과는 "n, n+1, n+2, n+3"이다.
                - "n년차 이하"이면 결과는 "0, 1, ..., n"이다.
                - "정확히 n년차"이면 결과는 "n"이다.
                - "신입" 또는 "경력 요구 없음"이면 결과는 "0"이다.
            예시출력:
            {
              "career": "1,2,3,4"
            }
        ...
        fun analysisCareerMessageFactory() : String {
            return analysisCareer
        }
    }
}